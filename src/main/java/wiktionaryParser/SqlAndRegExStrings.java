...
public  class SqlAndRegExStrings {
   ...
   public static String zeilenEnde="\\n";
    ...
    public static Pattern regexSimplestNewest = Pattern.compile("==[^(^=]*\\(\\{\\{Sprache\\|   (?<Sprache>[^}]*)   }}\\)\\ ?==" + zeilenEnde +
            "(?<allBefore>(?s:.)*?)  #Matched so wenig wie möglich also auch einen leerstring\n" +
            "(?:" +
            "\\{\\{Worttrennung}}" + zeilenEnde +
            ":(?<Worttrennung>[^,^\" + zeilenEnde + \"]*).*" + zeilenEnde +
            "(?s:.)*?" +
            ...
            "(?:\\[\\d,?–?\\ ?\\d\\])?" +
            "(?:\\{\\{Pl\\.}})?" +
            "\\ ?\\ ?\\{\\{Lautschrift\\|   (?<Lautschrift>[^}]*)   }}",Pattern.COMMENTS);


    public static Pattern regexSprache = Pattern.compile("==[^(^=]*\\(\\{\\{Sprache\\|   (?<Sprache>[^}]*)   }}\\)\\ ?==",Pattern.COMMENTS);


    public static Pattern regexIPA = Pattern.compile(
            "\\{\\{Aussprache}}" + zeilenEnde +
            ":\\{\\{IPA}}\\ ?" +
            "(?:\\[\\d,?–?\\ ?\\d\\])?" +
            "(?:\\{\\{Pl\\.}})?" +
